...
| `raw_manifest.json` | Машинно-читаемый индекс raw-экспорта по формату `docs/raw_manifest_spec.md`: schema version, demo path, версии пакетов, metadata/event/tick/error/control artifacts, размеры файлов, row counts, columns, failed records и fallback notes. |
...
4. `raw_manifest.json` является отдельным индексом результата экспорта и не требует изменения уже записанных raw CSV/JSON-артефактов.
5. Если parser вернул данные «как есть», эти значения сохраняются без изменения, исправления типов, переименования смыслов или заполнения пропусков на месте.
6. Все будущие scope.gg-подобные метрики должны явно ссылаться на source-файлы и поля raw-слоя, из которых они были рассчитаны.
