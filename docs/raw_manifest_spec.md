...
Документ фиксирует целевую структуру будущего manifest-файла raw-экспорта. Генерация `raw_manifest.json` пока не реализуется: спецификация нужна, чтобы следующий шаг был небольшим, проверяемым и не требовал заново обсуждать формат.
...
- Этот документ не требует менять `main.py`.
- Этот документ не требует создавать `raw_manifest.json` в существующем `debug_pack/`.
- Реализацию генерации manifest-файла нужно делать отдельным следующим изменением.
