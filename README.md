...
Скрипт читает следующие raw-источники, если они есть: `meta/player_info.csv`, события `player_death`, `player_hurt`, `weapon_fire`, `round_prestart`, `round_freeze_end`, `round_officially_ended`, `cs_win_panel_match`, objective-события бомбы и все `events/bomb_*.csv`. На первом этапе он создаёт таблицы:
